  ...
  <div class="relative z-10 max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    ...
    <header class="text-center mb-16 animate-fadeIn">
      <div class="inline-block">
        <h1 class="text-7xl font-black mb-4 tracking-tight gradient-text glow-text">
          Needleman-Wunsch
        </h1>
        <div class="h-1 w-32 mx-auto bg-gradient-to-r from-transparent via-sky-500 to-transparent rounded-full"></div>
      </div>
      <p class="text-xl text-slate-400 font-medium mt-6">
    ...
    <div class="grid lg:grid-cols-2 gap-8 mb-12">
      <!-- Input Panel -->
      <div class="glass-card rounded-2xl p-8 animate-slideUp border-slate-700/50 hover:border-sky-500/20 transition-all duration-300">
        <div class="flex items-center gap-4 mb-8">
          <div class="relative">
            <div class="w-12 h-12 bg-gradient-to-br from-sky-500 to-blue-600 rounded-xl flex items-center justify-center shadow-lg shadow-sky-500/20">
              <span class="text-white font-bold text-lg">01</span>
            </div>
            <div class="absolute inset-0 bg-sky-500/20 rounded-xl blur-xl"></div>
          </div>
          <h2 class="text-2xl font-bold text-slate-100">Input Parameters</h2>
        </div>
        ...
        <div class="space-y-6">
            ...
            <div class="group">
              ...
              <div class="relative">
                <input
                  ...
                  type="text"
                  bind:value={seq1}
                  class="w-full px-5 py-4 bg-slate-800/50 backdrop-blur border-2 border-slate-700/50 rounded-xl focus:ring-2 focus:ring-sky-500 focus:border-sky-500 transition-all duration-200 uppercase font-mono text-lg font-medium placeholder-slate-500 text-slate-100 hover:border-slate-600"
          ...
          <div class="grid grid-cols-3 gap-4 pt-2">
            <div class="group">
              ...
              <input
                ...
                class="w-full px-4 py-3 bg-slate-800/50 backdrop-blur border-2 border-slate-700/50 rounded-xl focus:ring-2 focus:ring-sky-500 focus:border-sky-500 transition-all duration-200 text-center font-bold text-slate-100 hover:border-slate-600"
              />
            </div>
          ...
          <button
            ...
            class="w-full bg-gradient-to-r from-sky-500 to-blue-600 hover:from-sky-400 hover:to-blue-500 text-white font-bold py-4 px-6 rounded-xl transition-all duration-200 transform hover:scale-[1.02] active:scale-[0.98] shadow-lg hover:shadow-sky-500/25 flex items-center justify-center gap-3 text-lg neon-button relative overflow-hidden"
          >
      ...
      {#if result && showAlignment}
        <div class="glass-card rounded-2xl p-8 animate-slideUp border-slate-700/50 hover:border-sky-500/20 transition-all duration-300" style="animation-delay: 0.1s;">
          <div class="flex items-center gap-4 mb-8">
            <div class="relative">
            ...
            <h2 class="text-2xl font-bold text-slate-100">Alignment Result</h2>
          </div>
          
          <div class="space-y-6">
            <!-- Aligned sequences -->
            <div class="bg-slate-800/50 backdrop-blur rounded-xl p-6 border border-slate-700/50">
              <h3 class="text-xs font-bold text-emerald-400 mb-4 uppercase tracking-wider">Aligned Sequences</h3>
            ...
            <!-- Score display -->
            <div class="bg-gradient-to-br from-emerald-500/10 to-teal-600/10 rounded-xl p-6 border border-emerald-500/20">
              <h3 class="text-xs font-bold text-emerald-400 mb-3 uppercase tracking-wider">Alignment Score</h3>
              <div class="flex items-center gap-4">
                <p class="text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-teal-400">{result.score}</p>
            ...
            <!-- Legend -->
            <div class="bg-slate-800/30 rounded-xl p-4 space-y-2 border border-slate-700/50">
              <h3 class="text-xs font-bold text-sky-400 mb-3 uppercase tracking-wider">Matrix Legend</h3>
              <div class="grid grid-cols-2 gap-3 text-sm">
                <div class="flex items-center gap-3">
                  <div class="w-4 h-4 bg-gradient-to-br from-sky-500/30 to-blue-600/30 rounded border border-sky-400/50 shadow-[0_0_8px_rgba(14,165,233,0.5)]"></div>
                  <span class="text-slate-400">Optimal path</span>
                </div>
                <div class="flex items-center gap-3">
                  <div class="w-4 h-4 bg-sky-500/20 rounded border border-sky-400 shadow-[0_0_8px_rgba(14,165,233,0.5)]"></div>
                  <span class="text-slate-400">Starting cell</span>
                </div>
                <div class="flex items-center gap-3">
                  <div class="w-4 h-4 bg-slate-800/30 rounded border border-slate-700/50"></div>
                  <span class="text-slate-400">Regular cell</span>
                </div>
                <div class="flex items-center gap-3">
                  <div class="w-4 h-4 bg-slate-700/50 rounded border border-sky-400/30"></div>
    ...
    {#if result && showAlignment}
      <div class="glass-card rounded-2xl p-8 animate-slideUp border-slate-700/50 hover:border-sky-500/20 transition-all duration-300" style="animation-delay: 0.2s;">
        <div class="flex items-center gap-4 mb-8">
          <div class="relative">
          ...
          <h2 class="text-2xl font-bold text-slate-100">Dynamic Programming Matrix</h2>
        </div>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  {#each seq2 as char}
                    <td class="w-20 h-20 text-center">
                      <div class="w-16 h-16 mx-auto bg-gradient-to-br from-slate-700 to-slate-800 text-sky-300 font-bold rounded-xl flex items-center justify-center text-lg shadow-lg border border-slate-600/50">
                        {char}
                      </div>
                    </td>
                  {/each}
                ...
                <tr>
                  ...
                  {#each result.matrix[0] as val, j}
                    <td class="w-20 h-20">
                      <div 
                        class="w-16 h-16 mx-auto border rounded-xl flex items-center justify-center font-bold transition-all duration-200 cursor-pointer matrix-cell {getCellClass(0, j)}"
                        onmouseenter={(e) => {
                          hoveredCell = [0, j];
                ...
                {#each result.matrix.slice(1) as row, i}
                  <tr>
                    ...
                    {#each row as val, j}
                      ...
                      <td class="w-20 h-20">
                        <div
                          class="w-16 h-16 mx-auto border rounded-xl flex items-center justify-center transition-all duration-200 cursor-pointer relative matrix-cell {getCellClass(actualI, actualJ)}"
                          onmouseenter={(e) => {
                        ...
                        >
                          ...
                          {#if getArrowDirection(actualI, actualJ)}
                            <span class="absolute top-1 left-1 text-xs text-sky-400 opacity-80">{getArrowDirection(actualI, actualJ)}</span>
                          {/if}
                        </div>
                      </td>
                    {/each}
...
{#if showTooltip && hoveredCell && result}
  <div 
    class="fixed z-50 glass-card text-sm rounded-xl py-4 px-5 pointer-events-none whitespace-pre-line shadow-2xl border border-sky-500/20 animate-fadeIn"
    style="left: {tooltipPosition.x}px; top: {tooltipPosition.y}px; transform: translate(-50%, -100%); min-width: 340px; background: rgba(17, 17, 19, 0.95); color: #e2e8f0;"
  >
    <div class="font-medium leading-relaxed text-slate-200">{getCellTooltip(hoveredCell[0], hoveredCell[1])}</div>
    <div class="absolute bottom-0 left-1/2 transform -translate-x-1/2 translate-y-full">
      <div class="w-0 h-0 border-l-8 border-r-8 border-t-8 border-transparent" style="border-top-color: rgba(17, 17, 19, 0.95);"></div>
    </div>
  </div>
{/if}